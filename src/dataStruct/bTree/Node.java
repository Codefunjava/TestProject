		...
		if (children.size() > tree.getOrder()) {
			...
			for (int i = 0; i < rightSize; i++) {
				...
				right.getEntries().add(
						new SimpleEntry(children.get(leftSize + i).getEntries()
								.get(0).getKey(), null));
	...
	protected static void validate(Node node, BplusTree tree) {
		...
		if (node.getEntries().size() == node.getChildren().size()) {
			for (int i = 0; i < node.getEntries().size(); i++) {
				Comparable key = node.getChildren().get(i).getEntries().get(0)
						.getKey();
			...
			if (isRoot) {
				...
				if (entries.size() > tree.getOrder() / 2 && entries.size() > 2) {
					...
					if (previous != null
						...
						Entry<Comparable, Object> entry = previous.getEntries()
								.get(size - 1);
						...
						if (previous != null
							...
							// 同后面节点合并
								...
								&& (next.getEntries().size() <= tree.getOrder() / 2 || next
										.getEntries().size() <= 2)
								&& next.getParent() == parent) {
